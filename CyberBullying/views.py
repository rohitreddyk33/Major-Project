...
def MonitorPost(request):
    if request.method == 'GET':
       ...
       con = pymysql.connect(host='127.0.0.1',port = 3308,user = 'root', password = '', database = 'cyber',charset='utf8')
       with con:
          cur = con.cursor()
...
def Signup(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3308,user = 'root', password = '', database = 'cyber',charset='utf8')
...
def PostSent(request):
    if request.method == 'POST' and request.FILES['t2']:
        ...
        filename = fs.save('D:/Cyber/CyberBullying/static/photo/'+str(myfile), myfile)
